...
author_wrote_book_at_age(Author,Title,Age):-
    book(Title,AuthorID,YearofR,_,_),
    author(AuthorID,Author,YearofB,_),
    Age is YearofR - YearofB.

youngest_author(Author):-
    author_wrote_book_at_age(Author,_,Age1),
    \+ (author_wrote_book_at_age(Author2,_,Age2),
       Age2 =< Age1,
       Author \= Author2).
...
diverse_books(Books):-
    findall(Title,(book(Title,_,_,_,Genres), length(Genres,L), \+ ((book(_,_,_,_,Genres2), length(Genres,L2), L2 > L))),Books).

country_authors(Country,Authors):-
    bagof(Author, (AuthorID, Year)^author(AuthorID,Author,Year,Country),Authors).
